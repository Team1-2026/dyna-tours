...
export const COUNTRIES_LIST: CountryOption[] = [
  ...
  { name: 'South Georgia and South Sandwich Islands', code: 'GS', flag: '🇬🇸' },
...
export const findCountryByName = (name?: string): CountryOption | undefined => {
  ...
  return COUNTRIES_LIST.find(c => c.name.toLowerCase() === clean);
};

export const findCountryByCode = (code?: string): CountryOption | undefined => {
  if (!code) return undefined;
  const clean = code.trim().toUpperCase();
  return COUNTRIES_LIST.find(c => c.code.toUpperCase() === clean);
};

export const getFlagForCountry = (nameOrCode?: string): string => {
  if (!nameOrCode) return '🏳️';
  const byName = findCountryByName(nameOrCode);
  if (byName) return byName.flag;
  const byCode = findCountryByCode(nameOrCode);
